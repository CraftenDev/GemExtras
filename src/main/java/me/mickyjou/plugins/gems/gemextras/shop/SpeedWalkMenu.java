...
public class SpeedWalkMenu extends SinglePageView {
    public SpeedWalkMenu(Player player) {
        ...
        addElement(new BuyButton(8, Material.GOLD_BOOTS, "2 hours") {
            @Override
            protected void onBuyItem(Player player) {
                GemExtras.getPlugin(GemExtras.class).getAbilityManager().giveAbilityTo(DoubleJump.class, player, Duration.ofHours(2));
            }
        });
        ...
        addElement(new BuyButton(20, Material.DIAMOND_BOOTS, "6 hours") {
            @Override
            protected void onBuyItem(Player player) {
                GemExtras.getPlugin(GemExtras.class).getAbilityManager().giveAbilityTo(DoubleJump.class, player, Duration.ofHours(6));
            }
        });
        ...
        addElement(new BuyButton(42, Material.CHAINMAIL_BOOTS, "24 hours") {
            @Override
            protected void onBuyItem(Player player) {
                GemExtras.getPlugin(GemExtras.class).getAbilityManager().giveAbilityTo(DoubleJump.class, player, Duration.ofHours(24));
            }
        });
    }
}
